...
function LibKeyBound:Initialize()
	do
		...
		f:SetScript('OnShow', function() PlaySound(856) end) -- SOUNDKIT.IG_MAINMENU_OPTION
		f:SetScript('OnHide', function() PlaySound(799) end) -- SOUNDKIT.GS_TITLE_OPTION_EXIT

		-- 7.3.5 compatibility: CreateTitleRegion was removed, make frame movable instead
		if f.CreateTitleRegion then
			local tr = f:CreateTitleRegion()
			tr:SetAllPoints(f)
		else
			-- In 7.3.5, manually make frame movable
			f:SetMovable(true)
			f:EnableMouse(true)
		...
		resetBindings:SetPoint('RIGHT', okayBindings, 'LEFT', -8, 0)  -- Move 8 pixels left with gap
	...
	StaticPopupDialogs["LIBKEYBOUND_RESET_KEYBINDS"] = {
		...
		OnAccept = function()
			if InCombatLockdown() then
				...
				-- Update the UI if the binding frame is open
