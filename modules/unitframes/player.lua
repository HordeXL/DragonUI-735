...
local function UpdateGroupIndicator()
    ...
    -- 7.3.5 compatibility: GetNumRaidMembers() replaced with GetNumGroupMembers()
    local numRaidMembers = GetNumGroupMembers and GetNumGroupMembers() or (GetNumRaidMembers and GetNumRaidMembers() or 0)
...
local function UpdateAlternateManaText()
    ...
    -- ⭐ 7.3.5 FIX: UnitPower powerType must be number (0 for mana)
    local currentMana = UnitPower("player", 0)  -- 0 = POWER_TYPE_MANA
...
local function CreatePlayerFrameTextures()
    ...
    if not dragonFrame.PlayerGroupIndicator then
        ...
        text:SetFont("Fonts\\ARKai_T.ttf", 9)
...
local function ApplyWidgetPosition()
    ...
    if not (widgetConfig and widgetConfig.edited) and ShouldShiftForOrderHall(Module.playerFrame, savedAnchor, posY) then
        local barHeight = OrderHallCommandBar:GetHeight() or 0
        posY = posY - barHeight
