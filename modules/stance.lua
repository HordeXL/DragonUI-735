...
if addon.DebugInfo then
	addon:DebugInfo("Stance", "姿态栏模块开始加载")
	-- Check critical APIs
	addon:CheckAPI("ShapeshiftBarFrame", "Stance")
	addon:CheckAPI("StanceBar", "Stance")
	addon:CheckAPI("NUM_SHAPESHIFT_SLOTS", "Stance")
	addon:CheckAPI("GetNumShapeshiftForms", "Stance")
end
...
-- 7.3.5 compatibility: Localize global variables for safe nil checks (legionDF pattern)
local NUM_SHAPESHIFT_SLOTS = NUM_SHAPESHIFT_SLOTS;  -- May be nil in 7.3.5
local ShapeshiftBar_Update = ShapeshiftBar_Update;  -- May not exist in 7.3.5
...
local function CreateStanceFrames()
    ...
    if addon.DebugInfo then
        addon:DebugInfo("Stance", "开始创建姿态栏框架")
    end
    ...
    if addon.TrackFrameCreation then
        addon:TrackFrameCreation('pUiStanceHolder', 'Frame', UIParent)
    end
    ...
    if addon.TrackFrameCreation then
        addon:TrackFrameCreation('pUiStanceBar', 'SecureHandlerStateTemplate', anchor)
    end
    
    -- Expose globally for compatibility
    _G.pUiStanceBar = stancebar
    
    if addon.DebugSuccess then
        addon:DebugSuccess("Stance", "姿态栏框架创建成功")
    end
...
local function stancebutton_update()
	...
	if not InCombatLockdown() then
	    local button1 = _G.ShapeshiftButton1 or _G.StanceButton1
	    if button1 then
		    button1:SetPoint('BOTTOMLEFT', anchor, 'BOTTOMLEFT', 0, 0)
	    end
	end
end
...
local function stancebutton_position()
    ...
    if addon.DebugInfo then
        addon:DebugInfo("Stance", "开始设置姿态栏按钮位置")
    end
	...
	-- 7.3.5 compatibility: Use localized NUM_SHAPESHIFT_SLOTS safely
	local maxSlots = NUM_SHAPESHIFT_SLOTS or (GetNumShapeshiftForms and GetNumShapeshiftForms()) or 10
	
	if addon.DebugInfo then
	    addon:DebugInfo("Stance", string.format("检测到最大姿态栏位: %d", maxSlots))
	    if GetNumShapeshiftForms then
	        local actualForms = GetNumShapeshiftForms()
	        addon:DebugInfo("Stance", string.format("当前角色实际姿态数: %d", actualForms))
	    end
	end
	...
	for index=1, maxSlots do
		local button = _G['ShapeshiftButton'..index] or _G['StanceButton'..index]
		if button then
		    ...
		    if index == 1 then
			    ...
			    local previous = _G['ShapeshiftButton'..index-1] or _G['StanceButton'..index-1]
			    if previous then
		    ...
		    if GetShapeshiftFormInfo then
		        ...
		        -- If GetShapeshiftFormInfo doesn't exist, show all buttons that exist
		        button:Show()
		        buttonsShown = buttonsShown + 1
		    end
		end
	...
	if addon.DebugInfo then
	    addon:DebugInfo("Stance", string.format("找到 %d 个姿态按钮, 显示 %d 个", buttonsFound, buttonsShown))
	end
...
local function stancebutton_updatestate()
	...
	-- ⭐ 7.3.5 FIX: NUM_SHAPESHIFT_SLOTS is nil, use GetNumShapeshiftForms()
	...
	for index=1, numForms do
		...
		if button and icon then
			...
			if isActive then
				-- 7.3.5 compatibility: ShapeshiftBarFrame may not exist
				if ShapeshiftBarFrame then
					ShapeshiftBarFrame.lastSelected = button:GetID()
				elseif StanceBar then
...
local function stancebutton_setup()
	...
	-- 7.3.5 compatibility: Use localized NUM_SHAPESHIFT_SLOTS
	local maxSlots = NUM_SHAPESHIFT_SLOTS or (GetNumShapeshiftForms and GetNumShapeshiftForms()) or 10
	for index=1, maxSlots do
...
local function ApplyStanceSystem()
    ...
    -- 7.3.5 compatibility: Use localized ShapeshiftBar_Update
...
local function RestoreStanceSystem()
    ...
    for index=1, maxSlots do
        ...
        if button then
            -- 7.3.5 compatibility: ShapeshiftBarFrame may not exist
            button:SetParent(ShapeshiftBarFrame or StanceBar or UIParent)
            button:ClearAllPoints()
	...
	for i = 1, maxSlots do
		...
		if button then
			...
			if i == 1 then
				...
				local prevButton = _G["ShapeshiftButton"..(i-1)] or _G["StanceButton"..(i-1)]
				if prevButton then
