...
if isAscensionServer then
    ...
    -- ⚠️ 7.3.5 版本按钮列表（增加了GuildMicroButton、EJMicroButton、StoreMicroButton）
    MICRO_BUTTONS = {
        _G.CharacterMicroButton,
        _G.SpellbookMicroButton,
        _G.TalentMicroButton,
        _G.AchievementMicroButton,
        _G.QuestLogMicroButton,
        _G.GuildMicroButton,        -- ⚠️ 新增：公会查找器
        _G.SocialsMicroButton,
        _G.LFDMicroButton,
        _G.CollectionsMicroButton,
        _G.PVPMicroButton,
        _G.EJMicroButton,            -- ⚠️ 新增：冒险指南
        _G.StoreMicroButton,         -- ⚠️ 新增：商城
        _G.MainMenuMicroButton,
        _G.HelpMicroButton
    }
    
    -- ⚠️ 打印调试日志，确认按钮是否存在
    addon:DebugInfo("MicroMenu", "========== 微型菜单按钮检测 ==========")
    addon:DebugInfo("MicroMenu", "GuildMicroButton: " .. tostring(_G.GuildMicroButton ~= nil))
    addon:DebugInfo("MicroMenu", "EJMicroButton: " .. tostring(_G.EJMicroButton ~= nil))
    addon:DebugInfo("MicroMenu", "StoreMicroButton: " .. tostring(_G.StoreMicroButton ~= nil))
end
...
local MicromenuAtlas = {
    ...
    -- ⚠️ 新增：冒险指南（AdventureGuide/EJ）的Atlas坐标
    ["UI-HUD-MicroMenu-AdventureGuide-Disabled"] = {0.000976562, 0.0634766, 0.00195312, 0.162109},
    ["UI-HUD-MicroMenu-AdventureGuide-Down"] = {0.000976562, 0.0634766, 0.166016, 0.326172},
    ["UI-HUD-MicroMenu-AdventureGuide-Mouseover"] = {0.000976562, 0.0634766, 0.330078, 0.490234},
    ["UI-HUD-MicroMenu-AdventureGuide-Up"] = {0.000976562, 0.0634766, 0.494141, 0.654297}
}
...
local function GetAtlasKey(buttonName)
    ...
    if isAscensionServer then
        ...
        buttonMap = {
            ...
            achievement = "UI-HUD-MicroMenu-Achievements",
            questlog = "UI-HUD-MicroMenu-Questlog",
            guild = "UI-HUD-MicroMenu-GuildCommunities",     -- ⚠️ 新增：公会查找器
            socials = "UI-HUD-MicroMenu-GuildCommunities",
            lfd = "UI-HUD-MicroMenu-Groupfinder",
            collections = "UI-HUD-MicroMenu-Collections",
            pvp = nil,
            ej = "UI-HUD-MicroMenu-AdventureGuide",          -- ⚠️ 新增：冒险指南
            store = "UI-HUD-MicroMenu-Shop",                 -- ⚠️ 新增：商城
            mainmenu = "UI-HUD-MicroMenu-GameMenu",          -- ⚠️ 修复：主菜单应该是GameMenu，不是Shop
            help = "UI-HUD-MicroMenu-GameMenu"
        }
    end
...
local function RestoreMicromenuSystem()
    ...
    if _G.pUiBagsBar then
        _G.pUiBagsBar.registeredInEditor = nil
    end
    if _G.pUiMicroMenu then
        _G.pUiMicroMenu.registeredInEditor = nil
    end
...
local function ApplyMicromenuSystem()
    ...
    -- 7.3.5 compatibility: KeyRingButton was removed (keyring feature removed)
    if KeyRingButton then
    ...
    function MainMenuMicroButtonMixin:bagbuttons_setup()
        ...
        -- Setup KeyRingButton (7.3.5 compatibility: may not exist)
        if KeyRingButton then
            ...
            local function SyncKeyRingButton()
                if KeyRingButton then
                    -- 7.3.5 compatibility: IsBagOpen(-2) may not work for keyring
                    local isOpen = false
                    if IsBagOpen then
                        isOpen = IsBagOpen(-2) or false
                    end
                    KeyRingButton:SetChecked(isOpen)
                end
            end

            -- 7.3.5 compatibility: ToggleKeyRing may not exist
        ...
        if not pUiBagsBar.toggleButton then
            addon:DebugInfo("BagsBar", "开始创建背包折叠按钮")
            
            pUiBagsBar.toggleButton = CreateFrame('Button', 'DragonUI_BagsToggle', UIParent)
            local toggleButton = pUiBagsBar.toggleButton
            toggleButton.toggle = false
            
            toggleButton:SetPoint("RIGHT", CharacterBag0Slot, "LEFT", 35, 0)
            ...
            toggleButton:SetScript("OnClick", function(self)
                if self.toggle then
                    ...
                    addon:DebugInfo("BagsBar", "背包折叠")
                end
                
                self.toggle = not self.toggle
            end)
            
            addon:DebugInfo("BagsBar", "背包折叠按钮创建完成")
            addon:DebugInfo("BagsBar", string.format("按钮可见性: %s", tostring(toggleButton:IsShown())))
            addon:DebugInfo("BagsBar", string.format("按钮父级: %s", toggleButton:GetParent() and toggleButton:GetParent():GetName() or "nil"))
            addon:DebugInfo("BagsBar", string.format("按钮FrameStrata: %s", toggleButton:GetFrameStrata()))
            addon:DebugInfo("BagsBar", string.format("按钮FrameLevel: %d", toggleButton:GetFrameLevel()))
            addon:DebugInfo("BagsBar", string.format("按钮Alpha: %.2f", toggleButton:GetAlpha()))
            addon:DebugInfo("BagsBar", string.format("按钮Size: %.1fx%.1f", toggleButton:GetSize()))
            
            addon:DebugInfo("BagsBar", string.format("pUiBagsBar可见性: %s", tostring(pUiBagsBar:IsShown())))
            addon:DebugInfo("BagsBar", string.format("pUiBagsBar Size: %.1fx%.1f", pUiBagsBar:GetSize()))
            local pX, pY = pUiBagsBar:GetCenter()
            addon:DebugInfo("BagsBar", string.format("pUiBagsBar中心坐标: (%.1f, %.1f)", pX or 0, pY or 0))
            addon:DebugInfo("BagsBar", string.format("pUiBagsBar Alpha: %.2f", pUiBagsBar:GetAlpha()))
            
            local btnX, btnY = toggleButton:GetCenter()
            addon:DebugInfo("BagsBar", string.format("按钮中心坐标: (%.1f, %.1f)", btnX or 0, btnY or 0))
            
            local bag0X, bag0Y = CharacterBag0Slot:GetCenter()
            addon:DebugInfo("BagsBar", string.format("CharacterBag0Slot中心坐标: (%.1f, %.1f)", bag0X or 0, bag0Y or 0))
            addon:DebugInfo("BagsBar", string.format("CharacterBag0Slot父级: %s", CharacterBag0Slot:GetParent() and CharacterBag0Slot:GetParent():GetName() or "nil"))
            addon:DebugInfo("BagsBar", string.format("CharacterBag0Slot FrameStrata: %s", CharacterBag0Slot:GetFrameStrata()))
            addon:DebugInfo("BagsBar", string.format("CharacterBag0Slot FrameLevel: %d", CharacterBag0Slot:GetFrameLevel()))
            
            local backpackX, backpackY = MainMenuBarBackpackButton:GetCenter()
            addon:DebugInfo("BagsBar", string.format("MainMenuBarBackpackButton中心坐标: (%.1f, %.1f)", backpackX or 0, backpackY or 0))
            addon:DebugInfo("BagsBar", string.format("MainMenuBarBackpackButton FrameLevel: %d", MainMenuBarBackpackButton:GetFrameLevel()))
            
            local point, relativeTo, relativePoint, x, y = toggleButton:GetPoint()
            addon:DebugInfo("BagsBar", string.format("按钮锚点: %s to %s/%s, offset: (%.1f, %.1f)",
                tostring(point),
    ...
    function MainMenuMicroButtonMixin:bagbuttons_refresh()
        ...
        -- 7.3.5 compatibility: HasKey may not exist
            ...
            if button then
                ...
                elseif isCharacterButton then
                    ...
                    if not button.DragonUIBackground then
                        ...
                        -- ⚠️ 修复：移除MainMenu的特殊排除，让所有按钮都使用相同的处理逻辑
                        button:SetScript('OnUpdate', function(self, elapsed)
                            -- Ensure timer is initialized
                            if not self.dragonUITimer then
                                self.dragonUITimer = 0
    ...
    function addon.RefreshBagsPosition()
        ...
        if bagsConfig and bagsConfig.scale then
        ...
        if frameInfo and frameInfo.frame then
            -- Aplicar posición desde database o usar default
            local bagsWidgetConfig = addon.db and addon.db.profile.widgets and addon.db.profile.widgets.bagsbar
            if bagsWidgetConfig and bagsWidgetConfig.anchor then
                frameInfo.frame:ClearAllPoints()
                frameInfo.frame:SetPoint(bagsWidgetConfig.anchor or "BOTTOMRIGHT", UIParent,
                    bagsWidgetConfig.anchor or "BOTTOMRIGHT", bagsWidgetConfig.posX or 1, bagsWidgetConfig.posY or 41)
            else
                frameInfo.frame:ClearAllPoints()
    ...
    function addon.RefreshBags()
        ...
        if addon.pUiArrowManager then
            ...
            -- ⭐ 7.3.5 compatibility: Check if textures exist
            if normal and pushed and highlight then
                if isCollapsed then
    ...
    -- ⭐ DEBUG: Add command to list micro menu buttons
    SlashCmdList["DRAGONUIMICROBUTTONS"] = function(msg)
        print("|cff00ff00=== 微型菜单按钮列表 ===")
        for i, button in ipairs(MICRO_BUTTONS) do
            if button then
                local name = button:GetName()
                local shown = button:IsShown() and "|cff00ff00显示|r" or "|cffff0000隐藏|r"
                print(string.format("|cffffcc00[%d]|r %s - %s", i, name or "未知", shown))
                
                -- Check if button has custom textures applied
                if button.DragonUIBackground then
                    print("  |cff00ff00✓ 已应用DragonUI样式|r")
                else
                    print("  |cffff0000✗ 未应用DragonUI样式|r")
                end
            end
        end
        print("|cff00ff00================================|r")
    end
    SLASH_DRAGONUIMICROBUTTONS1 = "/microbuttons"
    SLASH_DRAGONUIMICROBUTTONS2 = "/微型菜单"
    ...
    do
        ...
        if normal and pushed and highlight then
            ...
            pushed:set_atlas('bag-arrow-invert-2x')
            highlight:set_atlas('bag-arrow-invert-2x')
            normal:SetAllPoints(arrow)
            pushed:SetAllPoints(arrow)
            highlight:SetAllPoints(arrow)
            normal:Show()  -- 确保纹理显示
            pushed:Show()
            highlight:Show()
        end
    ...
    -- 7.3.5 compatibility: MiniMapLFG_UpdateIsShown may not exist
    if MiniMapLFG_UpdateIsShown and MiniMapLFGFrame then
    ...
    -- 7.3.5 compatibility: MiniMapLFGFrame may not exist
    if MiniMapLFGFrame then
    ...
    -- 7.3.5 compatibility: Check existence before accessing
    if LFDSearchStatus and MinimapBackdrop then
    ...
    -- 7.3.5 compatibility: Only hook if function exists
    if LFDSearchStatus and MiniMapLFGFrame then
        ReanchorLFDStatus()
        if LFDSearchStatus_Update then
    ...
    addon.package:RegisterEvents(function(self, event)
        ...
        updateFrame:SetScript("OnUpdate", function(self, dt)
            ...
            if elapsed >= 1 then
                ...
                -- 7.3.5 compatibility: HasKey may not exist
                if KeyRingButton and HasKey and HasKey() then
                    KeyRingButton:Show()
                end
    ...
    -- 关键修复: 在调用bagbuttons_setup之前设置背包按钮的parent
    -- 这样样式才能正确应用 (legionDF模式)
    if _G.pUiBagsBar then
        for _, bags in pairs(bagslots) do
            bags:SetParent(_G.pUiBagsBar)
        end
        MainMenuBarBackpackButton:SetParent(_G.pUiBagsBar)
    end
