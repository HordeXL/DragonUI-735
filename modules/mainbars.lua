...
local function InitializeMainbars()
   ...
   function MainMenuBarMixin:actionbutton_setup()
    ...
    local function ConnectBarsToEditor()
        ...
        -- ⚠️ 声望条在disableblizzard.lua中被彻底禁用
        -- 神器能量条保持存活作为数据源，但由 mainbars 的自定义 ArtifactBar 显示
        ...
        if addon.ActionBarFrames.artifactbar then
            addon:DebugInfo("ExpRepBar", "✅ 神器能量条容器已创建（数据源：MainMenuBarMaxLevelBar 保持存活）")
        end
    end
    ...
    -- ⚠️ 声望条在disableblizzard.lua中被彻底禁用
    -- 相关的SetPoint挂钩已移除
    -- 神器能量条MainMenuBarMaxLevelBar保持存活，作为ArtifactBar数据源
    ...
    -- 神器能量条由独立模块 artifactbar.lua 管理
    -- addon.RefreshArtifactBarPosition 由 artifactbar.lua 定义
